...
const ChargingMap = forwardRef<ChargingMapHandle, ChargingMapProps>(
  ({ onReady }, ref) => {
    ...
    const initMap = useCallback(async () => {
      ...
      // Leaflet needs the container to have explicit non-zero dimensions.
      // Force them here as a belt-and-suspenders guard.
      ...
      // CARTO light tiles — premium, minimal look
      L.tileLayer(
      ...
      // invalidateSize after a frame to handle any flex/grid layout that
      // resolved after the map was created with zero dimensions.
      requestAnimationFrame(() => {
        map.invalidateSize();
      });
      // Also invalidate after a short delay for slower layouts
      setTimeout(() => {
    ...
    useEffect(() => {
      ...
      // ResizeObserver keeps the map in sync if the container is resized
      // (e.g. panel open/close, window resize)
    ...
    useImperativeHandle(ref, () => ({
      setUserLocation(lat: number, lng: number) {
        ...
        import("leaflet").then(({ default: L }) => {
          ...
          // Planning-area marker — pulsing navy pin
      ...
      startSearchAnimation() {
        ...
        import("leaflet").then(({ default: L }) => {
          ...
          setTimeout(() => {
            if (mapRef.current) mapRef.current.flyTo([lat, lng], 13, { duration: 1.0, easeLinearity: 0.3 });
          }, 200);
        });
      },
      ...
      showResults(zones: ZoneDetail[], selected: string[]) {
        ...
        import("leaflet").then(({ default: L }) => {
              ...
              if (isSelected) {
                ...
                marker.bindPopup(L.popup({
                  ...
                  <div style="font-family:Times New Roman,serif;padding:20px 22px;min-width:220px;">
                    <div style="display:inline-flex;align-items:center;gap:6px;background:var(--color-navy-900);color:white;font-size:10px;letter-spacing:0.08em;padding:4px 10px;border-radius:99px;margin-bottom:14px;">
                      <svg width="8" height="8" viewBox="0 0 8 8" fill="none"><circle cx="4" cy="4" r="2.5" fill="white"/><circle cx="4" cy="4" r="4" stroke="white" stroke-width="0.7" fill="none" opacity="0.4"/></svg>
                      Top recommendation
                    </div>
                    <div style="font-size:26px;letter-spacing:-0.025em;color:var(--color-ink);margin-bottom:6px;">Site ${zone.label}</div>
                    <div style="font-size:13px;color:var(--color-ink-3);margin-bottom:16px;">${zone.latitude.toFixed(4)}°N, ${zone.longitude.toFixed(4)}°E</div>
                    <div style="border-top:1px solid var(--color-border);padding-top:14px;display:flex;flex-direction:column;gap:10px;">
                      <div style="display:flex;justify-content:space-between;align-items:center;">
                        <span style="font-size:12px;color:var(--color-ink-4);">Distance from area</span>
                        <span style="font-size:15px;color:var(--color-ink);">${distStr}</span>
                      </div>
                      <div style="display:flex;justify-content:space-between;align-items:center;">
                        <span style="font-size:12px;color:var(--color-ink-4);">Predicted demand</span>
                        <span style="font-size:15px;color:var(--color-ink);">${demandStr}</span>
                      </div>
                    </div>
                    <button onclick="window.open('https://www.google.com/maps/search/?api=1&query=${zone.latitude},${zone.longitude}','_blank')" style="margin-top:16px;width:100%;padding:10px;border-radius:10px;border:none;background:var(--color-navy-900);color:white;font-family:Times New Roman,serif;font-size:14px;cursor:pointer;">
                      View site on map →
                    </button>
                  </div>
                `));
                ...
                const candIcon = L.divIcon({
                  ...
                  html: `
                    <div style="
                      width:20px;height:20px;border-radius:50%;
                      background:white;border:2px solid rgba(10,22,40,0.35);
                ...
                marker.bindPopup(L.popup({
                  ...
                  <div style="font-family:Times New Roman,serif;padding:16px 18px;min-width:180px;">
                    ...
                    <div style="font-size:22px;color:var(--color-ink);letter-spacing:-0.02em;margin-bottom:4px;">Site ${zone.label}</div>
          ...
          // Fit to selected + planning area after all markers appear
    ...
    return (
      // The outer wrapper is position:relative with explicit width+height so
      // Leaflet always gets a non-zero bounding box on initialisation.
      // This wrapper participates in normal flow (fills its flex parent),
      // and the inner div is the actual Leaflet container.
      <div style={{ position: "relative", width: "100%", height: "100%" }}>
        <div
          ...
          style={{
            position: "absolute",
            inset: 0,
            // Background matches CARTO tile colour so there's no flash
            background: "#e8edf4",
          }}
        />
      </div>
